...
export const filterTooltips: FilterTooltipContent = {
  ...
  display: {
    title: '¿Qué tipo de pantalla elegir?',
    description:
...
export const usageOptions: FilterOption[] = [
  ...
  { value: 'diseno', label: 'Diseño', count: 15, icon: 'Palette' },
...
export const gamaOptions: FilterOption[] = [
  { value: 'entry', label: 'Básica', count: 10 },
  { value: 'media', label: 'Recomendada', count: 13 },
  { value: 'alta', label: 'Avanzada', count: 10 },
  { value: 'premium', label: 'Premium', count: 6 },
];

export const conditionOptions: FilterOption[] = [
  { value: 'nuevo', label: 'Nuevo', count: 30 },
  { value: 'reacondicionado', label: 'Reacondicionado', count: 7 },
...
function generateProducts(): CatalogProduct[] {
  ...
  const gamaDistribution: { gama: GamaTier; priceRange: [number, number]; quota: [number, number] }[] = [
    { gama: 'entry', priceRange: [1200, 2000], quota: [50, 90] },
    { gama: 'media', priceRange: [2000, 3500], quota: [90, 150] },
    { gama: 'alta', priceRange: [3500, 5500], quota: [150, 250] },
    { gama: 'premium', priceRange: [5500, 8000], quota: [250, 400] },
  ];
  ...
  brands.forEach((brand) => {
    for (let i = 0; i < brand.count; i++) {
      const gamaIndex = Math.floor(Math.random() * gamaDistribution.length);
      const gamaInfo = gamaDistribution[gamaIndex];
      const price = Math.floor(
        gamaInfo.priceRange[0] + Math.random() * (gamaInfo.priceRange[1] - gamaInfo.priceRange[0])
      );
      const quota = Math.floor(
        gamaInfo.quota[0] + Math.random() * (gamaInfo.quota[1] - gamaInfo.quota[0])
      );

      const processorInfo = processors[Math.random() > 0.6 ? 0 : 1];
      const processorModel = processorInfo.models[Math.floor(Math.random() * processorInfo.models.length)];

      const ramSizes = gamaInfo.gama === 'entry' ? [4, 8] : gamaInfo.gama === 'media' ? [8, 16] : [16, 32];
      const ramSize = ramSizes[Math.floor(Math.random() * ramSizes.length)];

      const storageSizes = gamaInfo.gama === 'entry' ? [256] : gamaInfo.gama === 'media' ? [256, 512] : [512, 1000];
      ...
      const refreshRates = gamaInfo.gama === 'premium' || gamaInfo.gama === 'alta' ? [60, 120, 144] : [60];
      const refreshRate = refreshRates[Math.floor(Math.random() * refreshRates.length)];

      const hasDiscount = Math.random() > 0.7;
      const discountPercent = hasDiscount ? Math.floor(Math.random() * 15) + 5 : 0;
      ...
      const product: CatalogProduct = {
        ...
        specs: {
          processor: {
            ...
            speed: `${(2.5 + Math.random()).toFixed(1)} GHz`,
          },
          ram: {
            size: ramSize,
            type: 'DDR4',
          ...
          storage: {
            size: storageSize,
            type: 'ssd',
            hasSecondSlot: Math.random() > 0.6,
          },
          display: {
            size: displaySize,
            resolution: gamaInfo.gama === 'entry' ? 'hd' : 'fhd',
            resolutionPixels: gamaInfo.gama === 'entry' ? '1366x768' : '1920x1080',
            type: gamaInfo.gama === 'premium' ? 'oled' : 'ips',
            refreshRate,
            touchScreen: Math.random() > 0.8,
          },
          gpu: {
            type: gamaInfo.gama === 'alta' || gamaInfo.gama === 'premium' ? 'dedicated' : 'integrated',
            brand: gamaInfo.gama === 'alta' || gamaInfo.gama === 'premium' ? 'NVIDIA' : processorInfo.brand === 'intel' ? 'Intel' : 'AMD',
            model: gamaInfo.gama === 'alta' || gamaInfo.gama === 'premium' ? 'GeForce RTX 3050' : processorInfo.brand === 'intel' ? 'Intel Iris Xe' : 'AMD Radeon',
            vram: gamaInfo.gama === 'alta' || gamaInfo.gama === 'premium' ? 4 : undefined,
          },
          connectivity: {
            wifi: Math.random() > 0.5 ? 'WiFi 6' : 'WiFi 5',
            bluetooth: '5.0',
            hasEthernet: Math.random() > 0.6,
          },
          ports: {
            usb: Math.floor(Math.random() * 2) + 2,
            usbC: Math.floor(Math.random() * 2) + 1,
            hdmi: Math.random() > 0.2,
            thunderbolt: gamaInfo.gama === 'premium',
            sdCard: Math.random() > 0.5,
            headphone: true,
          },
          keyboard: {
            backlit: gamaInfo.gama !== 'entry',
            numericPad: displaySize >= 15.6,
            language: 'Español Latino',
          },
          security: {
            fingerprint: gamaInfo.gama !== 'entry',
            facialRecognition: gamaInfo.gama === 'premium',
            tpmChip: true,
          },
          os: {
          ...
          battery: {
            capacity: `${Math.floor(40 + Math.random() * 30)}Wh`,
            life: `${Math.floor(6 + Math.random() * 6)} horas`,
          },
          dimensions: {
            weight: Number((1.5 + Math.random() * 1.5).toFixed(1)),
            thickness: Number((15 + Math.random() * 10).toFixed(1)),
          },
        },
