...
import { evaluateLandingAccess, evaluateFamilyFarmAccess } from '../services/landingApi';
...
// ── Family-Farm overlay gate (BAL-2521/BAL-2522, DEMO) ─────────────────────
//
// Clone of CadeOverlayGate's structure/steps. Whitelist-only (mirrors
// validate-dni, NO Equifax) via evaluateFamilyFarmAccess — does NOT reuse
// useDniValidation (that hook is hardwired to /validate-dni and must stay
// untouched). Sibling cross-flow is CADE-exact: a LINK to
// routes.catalogo(sibling_landing_slug), no re-validation, no dni/token
// crossed.
function FamilyFarmOverlayGate({ landing, onValidated }: { landing: string; onValidated: () => void; deadline?: string }) {
  const session = useSessionOptional();
