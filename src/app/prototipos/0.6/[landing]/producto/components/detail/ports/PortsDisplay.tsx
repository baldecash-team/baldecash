    ...
    <div className="w-full p-4 bg-gradient-to-r from-[rgba(var(--color-primary-rgb),0.05)] to-[rgba(var(--color-primary-rgb),0.1)] rounded-2xl border border-[rgba(var(--color-primary-rgb),0.2)]">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 text-center sm:text-left">
          ...
          <div>
            <h4 className="font-semibold text-[var(--text-strong,#111827)] font-['Asap',_sans-serif]">Ficha Técnica</h4>
            <p className="text-sm text-[var(--text-muted,#4b5563)] hidden sm:block">Descarga todas las especificaciones en PDF</p>
          </div>
        </div>
        <Button
...
export const PortsDisplay: React.FC<PortsDisplayProps> = ({
  ...
  const renderPort = (port: ProductPort, index: number) => {
    ...
    return (
      <div
        key={index}
        className="flex items-center gap-2 px-3 py-2.5 bg-[var(--surface-bg,#fafafa)] rounded-lg border border-[var(--border-soft,#e5e7eb)] min-h-[40px]"
      >
        <IconComponent className="w-4 h-4 text-[var(--color-primary)] flex-shrink-0" />
        <span className="text-sm font-medium text-[var(--text,#374151)] break-words">{port.name}</span>
        {port.count > 1 && (
          <span className="text-xs text-[var(--text-muted,#6b7280)] flex-shrink-0">×{port.count}</span>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="w-full bg-[var(--surface,#fff)] rounded-2xl p-6 shadow-sm border border-[var(--border-soft,#e5e7eb)]">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-[var(--text-strong,#111827)]">Puertos y Conectividad</h3>
          <p className="text-sm text-[var(--text-muted,#6b7280)]">Distribución de puertos en el equipo</p>
        </div>
      </div>
      ...
      <div className="relative flex items-start justify-center gap-4">
        ...
        <div className="flex flex-col gap-2 items-end sm:items-end flex-1 sm:flex-none sm:min-w-[140px]">
          <span className="text-xs font-semibold text-[var(--text-muted,#6b7280)] uppercase tracking-wider mb-2">Izquierda</span>
          {leftPorts.map((port, idx) => renderPort(port, idx))}
        </div>

        {/* Laptop Visual - Hidden on mobile */}
        <div className="hidden sm:flex flex-shrink-0 w-32 h-24 bg-[var(--surface-2,#f3f4f6)] rounded-lg items-center justify-center relative mt-6 border border-[var(--border-soft,#e5e7eb)]">
          <Laptop className="w-12 h-12 text-[var(--text-faint,#9ca3af)]" />
        ...
        <div className="flex flex-col gap-2 items-start flex-1 sm:flex-none sm:min-w-[140px]">
          <span className="text-xs font-semibold text-[var(--text-muted,#6b7280)] uppercase tracking-wider mb-2">Derecha</span>
      ...
      {backPorts.length > 0 && (
        <div className="mt-6 pt-4 border-t border-[var(--border-soft,#e5e7eb)]">
          <span className="text-xs font-semibold text-[var(--text-muted,#6b7280)] uppercase tracking-wider">Parte trasera</span>
      ...
      {bottomPorts.length > 0 && (
        <div className="mt-6 pt-4 border-t border-[var(--border-soft,#e5e7eb)]">
          <span className="text-xs font-semibold text-[var(--text-muted,#6b7280)] uppercase tracking-wider">Parte inferior</span>
          <div className="flex flex-wrap gap-2 mt-2 justify-center">
      ...
      <div className="mt-6 pt-4 border-t border-[var(--border-soft,#e5e7eb)]">
        <div className="flex flex-wrap gap-2 justify-center">
          ...
          <span className="px-3 py-1 bg-[var(--surface-2,#f3f4f6)] text-[var(--text-muted,#4b5563)] rounded-full text-xs font-medium">
