...
function Fila({ etiqueta, valor, destacado }: { etiqueta: string; valor: string; destacado?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 py-2 border-b border-slate-100 last:border-0">
      ...
      <span className={destacado ? 'text-sm font-semibold text-slate-900' : 'text-sm text-slate-900'}>
        {valor}
      </span>
    </div>
  );
...
export function DetalleFinanciamientoModal({ abierto, onCerrar, simulacion, montos }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <header className="sticky top-0 flex items-center justify-between gap-4 border-b border-slate-100 bg-white px-5 py-4">
          <h3 id="titulo-detalle-financiamiento" className="text-base font-semibold text-slate-900">
        ...
        <div className="px-5 py-4">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Detalles</h4>
        ...
        {simulacion && simulacion.cronograma.length > 0 && (
          <div className="px-5 py-4">
            <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Cronograma</h4>
                ...
                <tbody>
                  {simulacion.cronograma.map((cuota) => (
                    <tr key={cuota.numero} className="border-b border-slate-100 last:border-0">
                      <td className="py-2 text-slate-600">{formatearFecha(cuota.fechaVencimiento)}</td>
                      <td className="py-2 text-right text-slate-900">{formatearSoles(cuota.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
